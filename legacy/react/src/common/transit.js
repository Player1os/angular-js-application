...
const fixForHotReload = (Record, previous) => {
	...
	const transitGuidKey = names.find((name) => {
		return name.startsWith('transit$guid$');
	});
...
export const fromJSON = (string: string) => {
	return transitRecords().fromJSON(string);
}
export const toJSON = (object: any) => {
	return transitRecords().toJSON(object);
}
